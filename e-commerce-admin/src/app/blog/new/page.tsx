...
export default function NewBlogPage() {
  ...
  useEffect(() => {
    if (formData.authorId || authors.length === 0) return;
    const preferred =
      authors.find((author) => author.isActive && author.name === "Редакция Prime") ||
      authors.find((author) => author.isActive);
    if (preferred) setFormData((value) => ({ ...value, authorId: preferred.id }));
  }, [authors, formData.authorId]);
  ...
  return (
    <div className="space-y-4 lg:space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl lg:text-2xl font-semibold text-primary-black">Новая статья</h1>
          <p className="text-text-secondary-black mt-1">Создание новой статьи для блога</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 lg:space-y-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2"><Input required value={formData.slug} onChange={(e) => setFormData({ ...formData, slug: e.target.value })} /><Button type="button" variant="outline" onClick={generateSlug}>Сгенерировать</Button></div>
            </div>
        ...
        <Card>
          <CardHeader><CardTitle>Медиа и метаданные</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            ...
            <div><label className="block text-sm font-medium mb-2">Теги</label><Input value={formData.tags} onChange={(e) => setFormData({ ...formData, tags: e.target.value })} placeholder="Apple, iPhone, Технологии" /></div>
        ...
        <div className="flex gap-3 justify-end"><Button type="button" variant="outline" onClick={() => router.back()}>Отмена</Button><Button type="submit" variant="primary" disabled={createBlog.isPending}>{createBlog.isPending ? "Создание..." : "Создать статью"}</Button></div>
      </form>
    </div>
  );
}
