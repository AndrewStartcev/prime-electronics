  ...
  return (
    <div className="space-y-3">
      ...
      {items.map((item, index) => (
        <div key={index} className="grid gap-2 rounded-xl border border-gray-200 p-3 md:grid-cols-[1fr_1fr_auto]">
          {mode === "stats" ? (
            <>
              <Input placeholder="Значение" value={item.value || ""} onChange={(e) => { const next = [...items]; next[index] = { ...item, value: e.target.value }; onChange(next); }} />
              <Input placeholder="Подпись" value={item.label || ""} onChange={(e) => { const next = [...items]; next[index] = { ...item, label: e.target.value }; onChange(next); }} />
            </>
          ) : (
            <>
              <Input placeholder="Заголовок" value={item.title || ""} onChange={(e) => { const next = [...items]; next[index] = { ...item, title: e.target.value }; onChange(next); }} />
              <Input placeholder="Описание" value={item.text || ""} onChange={(e) => { const next = [...items]; next[index] = { ...item, text: e.target.value }; onChange(next); }} />
            </>
          )}
          <button type="button" className="flex h-10 w-10 items-center justify-center rounded-lg border border-gray-200 hover:bg-red-50 hover:text-red-600" onClick={() => onChange(items.filter((_, i) => i !== index))}><Trash2 className="h-4 w-4" /></button>
        </div>
      ))}
    </div>
  );
...
export default function PagesBuilderPage() {
  ...
  const addBlock = () => {
    setDraft((previous) => ({ ...previous, blocks: [...previous.blocks, { type: newBlockType, version: 1, data: JSON.parse(JSON.stringify(DEFAULT_DATA[newBlockType])) }] }));
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Не удалось сохранить страницу");
